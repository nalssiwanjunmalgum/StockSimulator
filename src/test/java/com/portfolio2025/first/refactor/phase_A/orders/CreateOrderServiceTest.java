...
/**
 * [수정] CheckIdempotencyPort -> IdempotencyPort로 변경함
 *
 */
...
class CreateOrderServiceTest {
    ...
    void limitBuyAccepted() {
        ...
        // 멱등/조회 포트 스텁
        when(idempotencyPort.tryClaim(eq(1L), eq(10L), eq("idem-LB-10"), anyString()))
                .thenReturn(IdempotencyPort.ClaimResult.CLAIM_SUCCESS);
        ...
        // idempotency: complete 호출 확인
        verify(idempotencyPort).complete(1L, 10L, "idem-LB-10", 111L, 222L);
        // findExisting는 호출 안 됨(충돌 없었으니까)
        verify(idempotencyPort, never()).findExisting(anyLong(), anyLong(), anyString());
    ...
    void marketSellAccepted() {
        ...
        when(idempotencyPort.tryClaim(eq(2L), eq(20L), eq("idem-MS-20"), anyString()))
                .thenReturn(IdempotencyPort.ClaimResult.CLAIM_SUCCESS);
        ...
        verify(idempotencyPort).complete(2L, 20L, "idem-MS-20", 333L, 444L);
    ...
    void duplicateIgnored() {
        ...
        when(idempotencyPort.tryClaim(eq(1L), eq(10L), eq("idem-LB-10"), anyString()))
                .thenReturn(IdempotencyPort.ClaimResult.CLAIM_CONFLICT);
        when(idempotencyPort.findExisting(1L, 10L, "idem-LB-10"))
                .thenReturn(Optional.of(new IdempotencyPort.Existing(
                        IdempotencyStatus.COMPLETED, 777L, 888L, "hash"))
                );
        ...
        verify(idempotencyPort, never()).complete(anyLong(), anyLong(), anyString(), anyLong(), anyLong());
    }
    ...
    void buyInsufficientCash() {
        ...
        verifyNoInteractions(saveOrderPort, saveStockOrderPort, publishOrderEventPort);
        // idempotency: tryClaim은 호출되었지만, 예외로 인해 complete는 호출되지 않아야 함
        verify(idempotencyPort).tryClaim(eq(1L), eq(10L), eq("idem-LB-10"), anyString());
        verify(idempotencyPort, never()).complete(anyLong(), anyLong(), anyString(), anyLong(), anyLong());
    }
    ...
    void invalidLimitPrice() {
        // NPE 관련 코드 어떻게? - 검증 메서드 추가하는 걸로 대체함
